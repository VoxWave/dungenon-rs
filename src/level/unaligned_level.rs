...
impl<T: Sync> UnalignedLevel<T> {
    ...
    pub fn add(&mut self, obj: Object<T>) -> bool {
        if self.collides((&obj.hitbox, obj.pos)) {
            false
        } else {
    ...
    pub fn collides<S>(&self, hitbox: (&S, Point<f32>)) -> bool
        where S: Shape + Send + Sync
    {
...
pub struct Object<T> {
    pub value: T,
    pub hitbox: Box<dyn Shape + Send + Sync>,
    pub pos: Point<f32>,
}
impl<T> Object<T> {
    pub fn new(value: T, hitbox: Box<dyn Shape + Send + Sync>, pos: Point<f32>) -> Self {
        Object { value, hitbox, pos }
    }

    pub fn collides(&self, hitbox: (&impl Shape, Point<f32>)) -> bool {
        gjk::collides((&self.hitbox, self.pos), hitbox)
    }
}
